...
class Alphabet(object):
	...
	def delete(self,label):
		"""delete the entry with the label, but this delete can only be used when all elements are added"""
		if label in self._label_to_index:
			index = self._label_to_index[label]
			del self._label_to_index[label]
			del self._index_to_label[index]
			self.num_labels -= 1
	def json_dumps(self):
